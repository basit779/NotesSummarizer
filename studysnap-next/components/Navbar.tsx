        ...
        <div className="glass-strong flex h-14 items-center justify-between rounded-2xl px-3 pr-2">
          ...
          {loading && !user ? (
            ...
            <nav className="flex items-center gap-1">
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      href={item.to}
                      className={cn(
                        'relative px-3 py-1.5 text-sm transition-colors rounded-lg cursor-pointer',
              ...
              <Link href="/billing">
                <MotionButton
                  ...
                  className="ml-1"
                >
                  {user.plan === 'PRO' ? (<><Sparkles className="h-3.5 w-3.5" /> Pro</>) : 'Upgrade'}
                </MotionButton>
              </Link>
              <button
            ...
            <nav className="flex items-center gap-1">
              ...
              <Link href="/login"><MotionButton size="sm" variant="ghost">Log in</MotionButton></Link>
              <Link href="/signup"><MotionButton size="sm" variant="primary">Get started</MotionButton></Link>
            </nav>
          )}
        </div>
      </motion.header>
