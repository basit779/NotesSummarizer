...
function UploadInner() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-white font-sans flex flex-col items-center">
      ...
      <div>
        ...
        <BlurFade delay={0.08}>
          <h1 className="mt-4 text-[36px] md:text-[46px] leading-[1.02] font-semibold tracking-[-0.025em] text-black">
            Drop a file.
            <span className="block text-black/35">Get notes.</span>
          </h1>
        </BlurFade>
        <BlurFade delay={0.16}>
          <p className="mt-4 text-[14.5px] text-black/55 leading-relaxed max-w-md">
      ...
      <BlurFade delay={0.24} className="mt-10">
        {canAddMore && (
          <motion.div
            ...
            initial={false}
            animate={{
              borderColor: isDragActive ? 'rgba(0,0,0,0.5)' : 'rgba(0,0,0,0.08)',
              scale: isDragActive ? 1.012 : 1,
            }}
            whileHover={{ scale: 1.004 }}
            transition={{ duration: 0.2, ease: 'easeOut' }}
            className={cn(
              'relative rounded-xl border overflow-hidden cursor-pointer bg-black/[0.012]',
              isDragActive && 'ring-4 ring-black/[0.08]',
              files.length === 0 ? 'px-8 py-14' : 'px-8 py-10',
            )}
          >
            {/* top hairline — intensifies on drag */}
            <div
              className={cn(
                'pointer-events-none absolute inset-x-8 top-0 h-px bg-gradient-to-r from-transparent via-black to-transparent transition-opacity duration-200',
                isDragActive ? 'opacity-40' : 'opacity-15',
              )}
              aria-hidden
            />
            <input {...getInputProps()} />
              ...
              >
                <div
                  className={cn(
                    ...
                    isDragActive ? 'border-black/40 bg-black/[0.06]' : 'border-black/[0.08] bg-black/[0.02]',
                  )}
                >
                  {files.length === 0 ? (
                    <UploadCloud
                      className={cn(
                        ...
                        isDragActive ? 'text-black' : 'text-black/60',
                      )}
                    />
                  ) : (
                    <Plus
            ...
            >
              <div className="flex items-center justify-between px-1 mono text-[10px] text-black/35 tracking-[0.2em] uppercase">
                ...
                {files.length > 1 && (
                  <button
                    onClick={() => setFiles([])}
                    disabled={stage !== 'idle'}
                    className="text-black/40 hover:text-black transition-colors duration-150 cursor-pointer disabled:opacity-40"
                  >
                    Clear all
                  </button>
                )}
              ...
              {files.map((file, i) => (
                <motion.div
                  ...
                  className="relative overflow-hidden rounded-xl border border-black/[0.08] bg-black/[0.015] hover:bg-black/[0.03] hover:border-black/[0.15] transition-colors duration-150"
                >
                  <div className="absolute left-0 top-3 bottom-3 w-[2px] rounded-r bg-black/70" aria-hidden />
                  <div className="flex items-center justify-between gap-3 pl-4 pr-2 py-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[8px] border border-black/[0.08] bg-black/[0.02]">
                        <FileText className="h-[15px] w-[15px] text-black/55" />
                      </div>
                    ...
                    <button
                      ...
                      disabled={stage !== 'idle'}
                      className="flex h-8 w-8 items-center justify-center rounded-[6px] text-black/40 hover:text-black hover:bg-black/[0.05] transition-colors duration-150 cursor-pointer disabled:opacity-40"
          ...
          {stage === 'processing' && (
            ...
            >
              ...
              <div className="mt-3 h-[2px] w-full rounded-full bg-black/[0.08] overflow-hidden relative">
                <motion.div
                  className="absolute inset-y-0 left-0 w-1/3 bg-gradient-to-r from-transparent via-black to-transparent"
                  animate={{ x: ['-100%', '300%'] }}
