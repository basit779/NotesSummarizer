...
export default function Results() {
  ...
  useEffect(() => {
    api.get(`/results/${id}`).then((r) => setResult(r.data.result)).catch(() => toast.error('Failed to load result'));
  }, [id]);
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-12">
      ...
      >
        <Link to="/history" className="mono text-xs text-white/40 hover:text-white inline-flex items-center gap-1.5 transition-colors cursor-pointer">
